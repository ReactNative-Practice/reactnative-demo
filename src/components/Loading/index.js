 ...
 * Loading 
 初始设置
    ...
    this.state = {
        type:'text', //必选，参数值参考上面5-9行
        animationType: 'fade', //可选，默认是fade，效果是渐进渐出显示和隐藏，其他可选参数 slide , none
        visible: false, //可选
        transparent: true, //可选
        time: 3000,  //可选 自定义 和 'always'，传'always'时，loading loading一直显示，若要在父级组件中回调函数关闭loading提示，可以用.hide()方法，() => { this.refs.loading.hide()}
        txt: 'null'  // text类型时必选，其他类型为可选(已经设置默认值)
    };

在父组件中用事件调用方法：
    1. 加载组件  
       import Loading from "../PACModule/PACCell/Loading";

    2. 引用组件
       render中return组件：
        <Loading ref="loading" /> 
        注意别漏了 ref="loading" ，不然会报错

    3. 在事件中传参调用
    <Button
        onPress={() => { this.refs.loading.show(
            {
              type:'netFail'
              }
          )}}
        title="按钮6（网络连接失败loading）"
        color="#FF6600"
        accessibilityLabel=""
      />
 */
...
export default class Loading extends Component {
    ...
    show(obj) {
        ...
        //默认是延迟3秒自动消失
        if (newObj.time !== 'always') {
            clearTimeout(timer);
            const timer = setTimeout(
                function () {
                    _this.setState({ visible: false });
                },
                newObj.time || _this.state.time
            );
        }
    }

    fail(text = '请求失败，求稍后重试', time = 2000) {
        this.show({ type: 'fail', txt: text, time: time })
    }

    success(text = '请求成功', time = 2000) {
        this.show({ type: 'success', txt: text, time: time })
    }

    loading() {
        this.show({ type: 'loading', time: 'always', })
    }
    ...
    render() {
        ...
        const { type, animationType, visible, transparent, time, txt } = this.props;
                    ...
                    >
                        <View style={styles.box}>
                            <View style={styles.imgLoading}>
                                <Image style={styles.imgStyle} source={require('./img/success.png')}></Image>
                                <Text style={styles.imgLoadingText}>
            ...
            case 'fail':
                return (
                    ...
                    >
                        <View style={styles.box}>
                            <View style={styles.imgLoading}>
                                <Image style={styles.imgStyle} source={require('./img/fail.png')}></Image>
                                <Text style={styles.imgLoadingText}>
