...
export default class RadioForm extends React.Component {
  constructor(props) {
    ...
    this.state = {
      is_active_index: '',
      is_active_value: this.props.initial,
  ...
  //   if (nextProps.radio_props != this.props.radio_props) {
  //     // if (nextProps.initial != '' && nextProps.radio_props.length > 0) {
  //     nextProps.radio_props.forEach((element, index) => {
  //       if (element.value == this.props.initial) {
  ...
  _renderButton(obj, i) {
    return (
      <RadioButton
        ...
        isSelected={this.state.is_active_value === obj.value}
        obj={obj}
        key={i}
        index={i}
        ...
        onPress={(value, index) => {
          ...
          this.setState({ is_active_index: index, is_active_value: value })
        }}
      />
    )
  }
...
export class RadioButtonInput extends React.Component {
  constructor(props) {
    ...
    this.state = {
      isSelected: false,
      buttonColor: props.buttonColor || '#299667'
    }
  }
  render() {
    var innerSize = { width: 10, height: 10, borderRadius: 10 / 2 }
    var outerSize = { width: 14 + 6, height: 14 + 6, borderRadius: (14 + 6) / 2 }
    if (this.props.buttonSize) {
      ...
      outerSize.width = this.props.buttonSize + 6
      outerSize.height = this.props.buttonSize + 6
      outerSize.borderRadius = (this.props.buttonSize + 6) / 2
    }
    ...
    var borderWidth = this.props.borderWidth || 1.2
...
RadioButtonInput.defaultProps = {
  buttonInnerColor: '#299667',
  buttonOuterColor: '#299667',
  disabled: false
}
